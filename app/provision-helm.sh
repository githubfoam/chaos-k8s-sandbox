...
HELM_ARCHIVE=helm-v3.9.0-linux-amd64.tar.gz
HELM_DIR=linux-amd64
HELM_BIN=$HELM_DIR/helm

# curl -LsO https://storage.googleapis.com/kubernetes-helm/$HELM_ARCHIVE && tar -zxvf $HELM_ARCHIVE \
# && chmod +x $HELM_BIN && cp $HELM_BIN /usr/local/bin


curl -LsO https://get.helm.sh/$HELM_ARCHIVE && tar -zxvf $HELM_ARCHIVE \
&& chmod +x $HELM_BIN && cp $HELM_BIN /usr/local/bin
